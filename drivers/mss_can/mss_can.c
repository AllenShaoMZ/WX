 ...
 *
 * SmartFusion2 microcontroller subsystem CAN bare metal software driver 
 * implementation.
 *
 * SVN $Revision: 7962 $
 * SVN $Date: 2015-10-09 17:45:33 +0530 (Fri, 09 Oct 2015) $
 */
...
)
{
    ...
    CAN_RXMSGOBJECT CANRxOBJ;

    /* Clear pending IRQs */
    SYSREG->SOFT_RST_CR |= SYSREG_CAN_SOFTRESET_MASK;
    NVIC_DisableIRQ(CAN_IRQn);
    NVIC_ClearPendingIRQ(CAN_IRQn);
    SYSREG->SOFT_RST_CR &= ~SYSREG_CAN_SOFTRESET_MASK;

    /* The platform only has one CAN controller */
    if (&g_can0 != this_can)
    {
        return(CAN_ERR);
    }
    ...
    {
        MSS_CAN_config_buffer_n(&g_can0, mailbox_number, &CANRxOBJ);
    }
    ...
    if (CAN_SPEED_MANUAL == bitRate)
    {
        ...
        if ((pcan_config->CFG_TSEG2 == 0u) ||
            ((pcan_config->SAMPLING_MODE == 1u) && (pcan_config->CFG_TSEG2 == 1u)))
        {
...
)
{
    ...
    //20200731
    //this_can->hw_reg->IntEbl.TX_MSG = ENABLE;
    ...
    this_can->hw_reg->IntEbl.TX_MSG = DISABLE;//ENABLE;
...
)
{
    ...
    if (this_can->hw_reg->TxMsg[mailbox_number].TXB.TXREQ == 0u)
    {
        ...
        //this_can->hw_reg->TxMsg[mailbox_number].TXB.L = pMsg->L | \
                                                   CAN_TX_WPNH_EBL | CAN_TX_INT_EBL | CAN_TX_REQ;
        this_can->hw_reg->TxMsg[mailbox_number].TXB.L = (pMsg->L | CAN_TX_WPNH_EBL | CAN_TX_WPNL_EBL | CAN_TX_INT_EBL);
...
)
{
    ...
    /* Set abort request */
    this_can->hw_reg->TxMsg[mailbox_number].TXB.L = (this_can->hw_reg->TxMsg[mailbox_number].TXB.L & ~CAN_TX_REQ) | CAN_TX_ABORT;
...
)
{
                             ...
                             mailbox_number < CAN_TX_MAILBOX; mailbox_number++)
    {
        ...
        if ((MSS_CAN_get_tx_buffer_status(this_can) & (1u<<mailbox_number)) == 0)
        {
            /* If the Tx buffer isn't busy.... */
            this_can->hw_reg->TxMsg[mailbox_number].ID = pMsg->ID;
            this_can->hw_reg->TxMsg[mailbox_number].DATALOW = pMsg->DATALOW;
...
)
{
    ...
    *status = this_can->hw_reg->ErrorStatus.L;
    ...
    return(((*status) >> CAN_ERROR_STATUS_SHIFT) & CAN_ERROR_STATUS_MASK);
}
...
/* Catch all ISR if no user supplied one */

#ifndef MSS_CAN_USER_ISR
/***************************************************************************//**
 * CAN interrupt service routine.
 * CAN_IRQHandler is included within the Cortex-M3 vector table as part of the
 * Fusion 2 CMSIS.
 */
#if defined(__GNUC__)
...
#endif
{
    ...
    ASSERT(!(uint8_t*)"An ISR is required here if interrupts are enabled");
#else
    ASSERT(!(uint8_t*)"Unexpected MSS CAN interrupt - MSS CAN NVIC Interrupts should be disabled");
#endif
}
#endif
